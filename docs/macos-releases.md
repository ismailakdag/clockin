...
The current version is **1.1.3 (7)**, for Apple Silicon and Intel. The app and the DMG are both Developer ID signed, notarized by Apple and stapled. The DMG contains the app, an Applications shortcut and the MIT license.
...
After 1.1.1, 1.1.2 and 1.1.3 were published, the live feed, the GitHub DMG and the website DMG were downloaded anonymously. They matched the build byte for byte, and the DMG and app passed `spctl` and `stapler validate`.
...
```sh
scripts/publish-mac.sh 1.1.4 --dry-run   # local rehearsal, nothing committed or published
scripts/publish-mac.sh 1.1.4             # the real release
```
