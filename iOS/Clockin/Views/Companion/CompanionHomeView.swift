...
struct CompanionHomeView: View {
    ...
    private var home: some View {
        ...
        .task(id: imageKey) {
            ...
            let decoded = await Task.detached(priority: .utility) {
                ...
                if let seated = WardrobeArt.decode("c01.png", folder: "Frames"),
                   let legs = WardrobeArt.workingLegs(from: seated) {
                    result["working-legs"] = WardrobeArt.recolor(legs, colorway: state.colorway)
                }
                return result
            }.value
    ...
    private func roomScene(_ room: WardrobeRoom) -> some View {
        ZStack(alignment: .topLeading) {
            ...
            if enabled || stateOverride != nil {
                if activity == .working, let legs = images["working-legs"] {
                    let rect = activity.legsRect(in: room, layout: state.homeLayout)
                    Image(decorative: legs, scale: 1).resizable().interpolation(.none)
                        .frame(width: rect.width, height: rect.height)
                        .position(x: rect.midX, y: rect.midY)
                }
                companion(room)
                if activity == .working { furniture(.desk, room: room, foreground: true) }
            }
    ...
    @ViewBuilder private func furniture(_ slot: WardrobeSlot, room: WardrobeRoom, foreground: Bool = false) -> some View {
        if let id = state.furniture[slot.rawValue], let item = WardrobeArt.home.items[id],
           item.slot == slot.rawValue, let anchor = room.slots[slot.rawValue], let image = images[item.file] {
            let origin = WardrobeGeometry.origin(pivot: item.pivot, anchor: anchor, degrees: 0)
            let x = state.homeLayout.mirrored ? 360 - origin.x - Double(image.width) : origin.x
            if !foreground && [.floorLeft, .floorRight, .desk].contains(slot) {
                Ellipse().fill(.black.opacity(0.16)).frame(width: Double(image.width) * 0.84, height: 9)
                    .position(x: x + Double(image.width)/2, y: origin.y + Double(image.height) - 2)
            }
            CompanionFurnitureImage(image: image, mirrored: state.homeLayout.mirrored,
                                    swaying: moving && ["potted-plant", "big-plant"].contains(id))
                .frame(width: CGFloat(image.width), height: CGFloat(image.height))
                .mask(alignment: .bottom) {
                    Rectangle().frame(height: foreground ? max(0, Double(image.height) - item.pivot.y) : Double(image.height))
                }
                .offset(x: x, y: origin.y)
        }
    }
    ...
    private func companion(_ room: WardrobeRoom) -> some View {
        let center = activity.center(in: room, layout: state.homeLayout)
        return Group {
        ...
        .mask(alignment: .top) {
            Rectangle().frame(height: activity == .working
                ? max(0, (room.slots["desk"]?.y ?? 158) - center.y + activity.side / 2)
                : activity.side)
        }
