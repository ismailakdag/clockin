...
struct DashboardPinnedTools: View {
    ...
    var body: some View {
        if chimePinned { DashboardChimeCard { open(.chime) } }
        FocusRadioCard()
...
private struct DashboardChimeCard: View {
    let adjust: () -> Void
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            FocusChimeToggle().font(.headline)
            Stepper(value: $interval, in: 1...120) {
                Text("Every \(interval) min of work").font(.subheadline)
            }.disabled(!enabled).accessibilityIdentifier("chime.interval")
            HStack {
                Button(action: adjust) {
                    Label("\(FocusChimeSound.selected(sound).displayName) · Sound & volume", systemImage: "slider.horizontal.3")
                        .font(.caption).frame(minHeight: 44)
                }.accessibilityIdentifier("chime.adjust")
                Spacer(minLength: 0)
                Menu {
