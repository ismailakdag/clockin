...
struct PNG {
    ...
    init(_ url: URL) {
        require(fm.fileExists(atPath: url.path), "Missing image \(url.path)")
...
struct Colorway: Decodable { let name:String,map:[String:String] }
...
let ways=try decode([String:Colorway].self,frames.appendingPathComponent("colorways.json"))
require(Set(["classic","midnight","mint","sunset","gold","stealth"]).isSubset(of:Set(ways.keys)),"Missing colorways")
for (id,way) in ways {
    var mapped=Set<Int>()
    require(!way.name.isEmpty,"Empty colorway name")
    for (from,to) in way.map {
        guard let source=rgbValue(from),let target=rgbValue(to) else { fatalError("Malformed RGB in \(id)") }
        require(sourceColors.contains(source),"Unknown source color: \(id) \(from)")
        mapped.insert(source)
        if id=="classic" { require(source==target,"Classic is not identity") }
        if max((source>>16)&255,(source>>8)&255,source&255)<80 { require(source==target,"Black visor changed: \(id) \(from)") }
    }
    require(mapped==sourceColors,"Incomplete exact palette: \(id)")
}
print("ok: \(ways.count) colorways, \(sourceColors.count) existing source colors each; classic identity and visor preserved")
