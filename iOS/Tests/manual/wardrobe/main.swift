...
check(CompanionHomeActivity.resolve(working: true, paused: false, elapsed: 9*3600, tired: true, furniture: fullRoom) == .working, "long active session stays at desk")
...
check(CompanionHomeActivity.resolve(working: false, paused: true, elapsed: 8*3600, tired: true, furniture: [:]) == .idle, "missing furniture falls back to standing")
...
@MainActor func homeProof(roomID: String, activity: CompanionHomeActivity, layout: CompanionHomeLayout) {
    let room = realHome.rooms[roomID]!
    var outfit = WardrobeState()
    outfit.equipped = ["head":"beanie", "neck":"scarf"]
    let furniture = ["round-rug", "poster", "wall-clock", "bookshelf", "desk-monitor", "potted-plant", "coffee-machine",
    ...
    for id in furniture {
        ...
        let p=WardrobeGeometry.origin(pivot:item.pivot,anchor:room.slots[item.slot]!,degrees:0)
        let x=layout.mirrored ? 360-p.x-Double(image.width):p.x
        if ["floorLeft","floorRight","desk"].contains(item.slot) {
            context.setFillColor(CGColor(gray:0,alpha:0.16))
            context.fillEllipse(in:CGRect(x:x+Double(image.width)*0.08,y:p.y+Double(image.height)-6,width:Double(image.width)*0.84,height:9))
        }
        draw(image,x:x,y:p.y,w:Double(image.width),h:Double(image.height),mirror:layout.mirrored)
    }
    if activity == .working {
        let legs = WardrobeArt.workingLegs(from: realImage("Frames/c01.png"))!
        let rect = activity.legsRect(in: room, layout: layout)
        draw(legs,x:rect.minX,y:rect.minY,w:rect.width,h:rect.height)
    }
    ...
    let robot=WardrobeArt.composite(robot:realImage("Frames/"+frame+".png"),parts:overlays,size:314)!
    let center=activity.center(in:room,layout:layout),side=activity.side
    context.saveGState()
    if activity == .working { context.clip(to:CGRect(x:0,y:0,width:360,height:room.slots["desk"]!.y)) }
    ...
    draw(robot,x:-side/2,y:-side/2,w:side,h:side)
    context.restoreGState()
    if activity == .working {
        let item=realHome.items["desk-monitor"]!,image=realImage("Home/"+item.file),anchor=room.slots["desk"]!
        let p=WardrobeGeometry.origin(pivot:item.pivot,anchor:anchor,degrees:0)
        context.saveGState();context.clip(to:CGRect(x:0,y:anchor.y,width:360,height:240-anchor.y))
        draw(image,x:layout.mirrored ? 360-p.x-Double(image.width):p.x,y:p.y,w:Double(image.width),h:Double(image.height),mirror:layout.mirrored)
        context.restoreGState()
    }
    savePNG(context.makeImage()!, "/tmp/clockin-home-"+roomID+"-"+activity.rawValue+"-"+layout.rawValue+".png")
}
for roomID in realHome.rooms.keys.sorted() {
    for activity in CompanionHomeActivity.allCases {
        for layout in CompanionHomeLayout.allCases { homeProof(roomID:roomID,activity:activity,layout:layout) }
    }
}
...
for room in realHome.rooms.values {
    for layout in CompanionHomeLayout.allCases {
        let rect = CompanionHomeActivity.working.legsRect(in: room, layout: layout)
        check(rect.minY > room.slots["desk"]!.y && rect.maxY <= 224, "seated legs below desktop and above floor")
    }
}
